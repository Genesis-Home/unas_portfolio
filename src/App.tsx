...
import { Mail, MapPin, Github as GitHub, Linkedin as LinkedIn, Terminal, Code2 } from 'lucide-react';
...
function App() {
  return (
    <div className="min-h-screen bg-white">
      ...
      <footer id="contact" className="py-32 bg-slate-50 relative overflow-hidden">
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid lg:grid-cols-[1fr_1.5fr] gap-20 items-center">
              <div>
                ...
                <p className="text-slate-600 text-lg mb-12 leading-relaxed font-light">
                  Currently open to senior-level opportunities and high-impact architectural consultancy. Let's build something exceptional.
                </p>
                ...
                <div className="flex flex-col gap-6">
                  <div className="group flex items-center gap-5 p-6 rounded-[32px] bg-white border border-slate-200 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 transition-all">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Email Terminal</p>
                      <a href="mailto:Unas.engineer@gmail.com" className="text-lg font-bold text-slate-900">Unas.engineer@gmail.com</a>
                    </div>
                  </div>
                  ...
                  <div className="group flex items-center gap-5 p-6 rounded-[32px] bg-white border border-slate-200 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 transition-all">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-slate-900">Pakistan / Remote World</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                {[
                  { label: 'LinkedIn', icon: <LinkedIn size={24} />, link: 'https://linkedin.com/in/unas/', color: 'hover:text-[#0077b5]' },
                  { label: 'GitHub', icon: <GitHub size={24} />, link: 'https://github.com', color: 'hover:text-black' },
                  { label: 'Architecture', icon: <Code2 size={24} />, link: '#', color: 'hover:text-primary' },
                  { label: 'Résumé', icon: <Terminal size={24} />, link: '#', color: 'hover:text-indigo-600' }
                ].map((social, i) => (
                  <a 
                    key={i}
                    href={social.link}
                  ...
                  >
                    <div className={`text-slate-500 ${social.color} transition-all duration-300 group-hover:scale-125`}>
                      {social.icon}
                    </div>
                    <span className="text-sm font-bold text-slate-900 group-hover:tracking-[0.2em] transition-all">{social.label}</span>
                  </a>
                ))}
              </motion.div>
            </div>
            ...
            <div className="mt-32 pt-12 border-t border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="flex gap-8 text-[10px] font-bold text-slate-600 uppercase tracking-[0.2em]">
                <a href="#hero" className="hover:text-primary transition-colors">Architecture</a>
                <a href="#skills" className="hover:text-primary transition-colors">Tech Stack</a>
                <a href="#experience" className="hover:text-primary transition-colors">Operations</a>
              </div>
            </div>
          </div>
        </div>
